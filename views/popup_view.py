...
class PopupView(QWidget):
    def __init__(self, parent_widget, product):
        ...
        self.are_you_sure_label = QLabel(
            "Passe o seu token RFDI para comprar '{}' por R${:.2f}\n{}s".format(self.product.name, self.product.price,
                                                                                self.seconds_remaining))
        self.are_you_sure_label.setAlignment(Qt.AlignCenter)
        self.are_you_sure_label.setStyleSheet(
            "background-color: rgba(0, 0, 0, 0); border: none; font-size: 18px; font-weight: bold; color: white")

        # No button
        self.no_button = QPushButton("Cancelar")
        self.no_button.clicked.connect(self.handleCancelPurchase)
        self.no_button.setStyleSheet(
            "background-color: rgba(0, 0, 0, 0); border: none; font-size: 32px; font-weight: bold; color: white")
    ...
    def update(self):
        ...
        self.are_you_sure_label.setText(
            "Passe o seu token RFDI para comprar '{}' por R${:.2f}?\n{}s".format(self.product.name, self.product.price,
                                                                                 self.seconds_remaining))
